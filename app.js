...
function setupReadOnlyMode() {
    ...
    actionButtons.forEach(btn => {
        ...
        const parent = btn.closest('tr, .form-actions, .page-header, tbody, thead');
        if (parent) {
    ...
    // Masquer les colonnes "Actions" dans les tableaux si elles ne contiennent que des boutons d'édition/suppression
    const actionColumns = document.querySelectorAll('th:last-child, td:last-child');
        ...
        if (col.textContent.trim().toLowerCase() === 'actions') {
            ...
            if (table) {
                ...
                rows.forEach(row => {
                    ...
                    if (lastCell && lastCell.textContent.trim().toLowerCase() === 'actions') {
                        // Ne pas masquer si c'est un en-tête
                        if (lastCell.tagName === 'TH') return;
                        // Vérifier si la cellule ne contient que des boutons d'édition/suppression
                        const buttons = lastCell.querySelectorAll('button');
                        const hasOnlyEditDelete = Array.from(buttons).every(btn => {
                        ...
                        if (hasOnlyEditDelete && buttons.length > 0) {
...
async function fetchDataFromSupabase() {
    ...
    try {
        ...
        // Récupérer toutes les données en parallèle depuis Supabase
        const [recipesResponse, taxisResponse, driversResponse, commentsResponse] = await Promise.all([
            fetch(`${SUPABASE_CONFIG.URL}/rest/v1/recipes?select=*&order=date.desc`, {
                headers: {
                    'apikey': SUPABASE_CONFIG.ANON_KEY,
                    'Authorization': `Bearer ${SUPABASE_CONFIG.ANON_KEY}`
                }
            }).then(r => r.json()).catch(() => []),
            fetch(`${SUPABASE_CONFIG.URL}/rest/v1/taxis?select=*`, {
                headers: {
                    'apikey': SUPABASE_CONFIG.ANON_KEY,
                    'Authorization': `Bearer ${SUPABASE_CONFIG.ANON_KEY}`
                }
            }).then(r => r.json()).catch(() => []),
            fetch(`${SUPABASE_CONFIG.URL}/rest/v1/drivers?select=*`, {
                headers: {
                    'apikey': SUPABASE_CONFIG.ANON_KEY,
                    'Authorization': `Bearer ${SUPABASE_CONFIG.ANON_KEY}`
                }
            }).then(r => r.json()).catch(() => []),
            fetch(`${SUPABASE_CONFIG.URL}/rest/v1/comments?select=*&order=month.desc`, {
                headers: {
                    'apikey': SUPABASE_CONFIG.ANON_KEY,
                    'Authorization': `Bearer ${SUPABASE_CONFIG.ANON_KEY}`
                }
...
async function addRecipe(recipe) {
    ...
    if (currentRole === 'gestionnaire') {
        try {
            ...
            const response = await fetch(`${SUPABASE_CONFIG.URL}/rest/v1/recipes`, {
                method: 'POST',
                headers: {
                    'apikey': apiKey,
            ...
            if (!response.ok) {
                ...
                console.error('Erreur Supabase response:', response.status, errorData);
                throw new Error(errorData.message || errorData.error_description || `Erreur ${response.status} lors de l'ajout dans Supabase`);
            }

            const result = await response.json();
            recipe.id = result[0]?.id || recipe.id;
...
async function updateRecipe(id, recipe) {
    ...
    if (currentRole === 'gestionnaire') {
        try {
            const apiKey = getSupabaseApiKey();
            const response = await fetch(`${SUPABASE_CONFIG.URL}/rest/v1/recipes?id=eq.${id}`, {
                method: 'PATCH',
                headers: {
                    'apikey': apiKey,
                    'Authorization': `Bearer ${apiKey}`,
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Erreur lors de la mise à jour dans Supabase');
            }
...
async function deleteRecipe(id) {
    ...
    if (currentRole === 'gestionnaire') {
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                throw new Error(errorData.message || 'Erreur lors de la suppression dans Supabase');
            }
...
async function addTaxi(taxi) {
    ...
    if (currentRole === 'gestionnaire') {
        try {
            const apiKey = getSupabaseApiKey();
            const response = await fetch(`${SUPABASE_CONFIG.URL}/rest/v1/taxis`, {
                method: 'POST',
                headers: {
                    'apikey': apiKey,
                    'Authorization': `Bearer ${apiKey}`,
...
async function updateTaxi(id, taxi) {
    ...
    if (currentRole === 'gestionnaire') {
        try {
            const apiKey = getSupabaseApiKey();
            const response = await fetch(`${SUPABASE_CONFIG.URL}/rest/v1/taxis?id=eq.${id}`, {
                method: 'PATCH',
                headers: {
                    'apikey': apiKey,
                    'Authorization': `Bearer ${apiKey}`,
            ...
            showToast('Taxi mis à jour avec succès dans Supabase!', 'success');
        } catch (error) {
...
async function addDriver(driver) {
    ...
    if (currentRole === 'gestionnaire') {
        try {
            const apiKey = getSupabaseApiKey();
            const response = await fetch(`${SUPABASE_CONFIG.URL}/rest/v1/drivers`, {
                method: 'POST',
                headers: {
                    'apikey': apiKey,
                    'Authorization': `Bearer ${apiKey}`,
            ...
            const result = await response.json();
            driver.id = result[0]?.id || driver.id;
...
async function updateDriver(id, driver) {
    ...
    if (currentRole === 'gestionnaire') {
        try {
            const apiKey = getSupabaseApiKey();
            const response = await fetch(`${SUPABASE_CONFIG.URL}/rest/v1/drivers?id=eq.${id}`, {
                method: 'PATCH',
                headers: {
                    'apikey': apiKey,
                    'Authorization': `Bearer ${apiKey}`,
