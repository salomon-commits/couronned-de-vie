...
function parseCSV(csvText) {
    try {
        if (!csvText || typeof csvText !== 'string') {
            console.warn('parseCSV: texte CSV invalide');
            return [];
        }
        ...
        for (let i = 1; i < lines.length; i++) {
            try {
                ...
                if (values.length > 0 && (values[0] || values.some(v => v))) {
                    const obj = {};
                ...
                console.warn(`Erreur parsing ligne ${i}:`, error);
        ...
        console.error('Erreur parseCSV:', error);
        return [];
    }
}
...
async function fetchDataFromGoogleSheets() {
    ...
    try {
        ...
        // Parser les CSV avec validation
        try {
            const recipesParsed = parseCSV(recipesData);
            allData.recipes = Array.isArray(recipesParsed) 
                ? recipesParsed
                    .filter(r => r && typeof r === 'object')
                    .map((r, index) => ({
                        id: parseInt(r.ID) || index + 1,
                        date: (r.Date || '').toString().trim(),
                        matricule: (r.Matricule || '').toString().trim(),
                        recetteNormale: (() => {
                            const val = r['Recette Normale (FCFA)'] || '';
                            const cleaned = val.toString().replace(/[^\d.,]/g, '').replace(',', '.');
                            return parseFloat(cleaned) || 0;
                        })(),
                        montantVerse: (() => {
                            const val = r['Montant Versé (FCFA)'] || '';
                            const cleaned = val.toString().replace(/[^\d.,]/g, '').replace(',', '.');
                            return parseFloat(cleaned) || 0;
                        })(),
                        chauffeur: (r.Chauffeur || '').toString().trim(),
                        typeCourse: (r['Type de Course'] || 'ville').toString().trim(),
                        remarques: (r.Remarques || '').toString().trim(),
                        timestamp: r.Timestamp ? parseInt(r.Timestamp) : new Date().getTime()
                    }))
                    .filter(r => r.id && r.matricule) // Filtrer les entrées invalides
                : [];
        } catch (error) {
            console.error('Erreur parsing recipes:', error);
            allData.recipes = [];
        }
        
        try {
            const taxisParsed = parseCSV(taxisData);
            allData.taxis = Array.isArray(taxisParsed)
                ? taxisParsed
                    .filter(t => t && typeof t === 'object')
                    .map((t, index) => ({
                        id: parseInt(t.ID) || index + 1,
                        matricule: (t.Matricule || '').toString().trim(),
                        marque: (t['Marque/Modèle'] || '').toString().trim(),
                        proprietaire: (t.Propriétaire || 'COURONNE DE VIE').toString().trim()
                    }))
                    .filter(t => t.id && t.matricule) // Filtrer les entrées invalides
                : [];
        } catch (error) {
            console.error('Erreur parsing taxis:', error);
            allData.taxis = [];
        }
        
        try {
            const driversParsed = parseCSV(driversData);
            allData.drivers = Array.isArray(driversParsed)
                ? driversParsed
                    .filter(d => d && typeof d === 'object')
                    .map((d, index) => ({
                        id: parseInt(d.ID) || index + 1,
                        nom: (d.Nom || '').toString().trim(),
                        telephone: (d.Téléphone || '').toString().trim(),
                        taxiAssocie: (d['Taxi Associé'] || '').toString().trim(),
                        photo: (d['Photo URL'] || '').toString().trim() || null
                    }))
                    .filter(d => d.id && d.nom) // Filtrer les entrées invalides
                : [];
        } catch (error) {
            console.error('Erreur parsing drivers:', error);
            allData.drivers = [];
        }
        
        try {
            const commentsParsed = parseCSV(commentsData);
            allData.comments = Array.isArray(commentsParsed)
                ? commentsParsed
                    .filter(c => c && typeof c === 'object')
                    .map(c => ({
                        month: (c.Mois || '').toString().trim(),
                        comments: (c.Commentaires || '').toString().trim(),
                        dateCreation: (c['Date Création'] || '').toString().trim()
                    }))
                    .filter(c => c.month) // Filtrer les entrées invalides
                : [];
        } catch (error) {
            console.error('Erreur parsing comments:', error);
            allData.comments = [];
        }
